...
def load_saved_model(saved_path, model):
    ...
    if initial_epoch > 0:
        ...
        # if initial_epoch == 1 and model.has_trained is not None and model.has_trained is True:
        #     simple_copy_to_teacher(model)
        del checkpoint

    return initial_epoch, model

def simple_copy_to_teacher(model):
    with torch.no_grad():
        for (name, param), (t_name, t_param) in zip(
            ...
            if not name.startswith('teacher.'):
                ...
                print(f"Copied: {name} -> teacher.{t_name}")
