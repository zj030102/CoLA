        ...
        for i, batch_data_list in enumerate(train_loader):
            ...
            if not opt.half:
                ...
                final_loss = criterion(output_dict,
                                       batch_data_list[0]['ego']['label_dict'])
            else:
                with torch.cuda.amp.autocast():
                    output_dict = model(batch_data_list)
                    final_loss = criterion(output_dict,
                                        batch_data_list[0]['ego']['label_dict'])
            print('At epoch %d iter %d, the total loss is %f' % (epoch, i, final_loss))

            if not opt.half:
                final_loss.backward()

                for name, param in model.flow_generator.named_parameters():
                    if param.grad is not None:
                        print(f'grad/{name}: {param.grad.abs().mean().item():.4e}')
                        break
