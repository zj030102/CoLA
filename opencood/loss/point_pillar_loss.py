...
class PointPillarLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        if 'sim_loss' in output_dict:
            print(f"detection loss is {total_loss}")
            total_loss = total_loss + 0.1*output_dict['sim_loss']
            print(f"sim_loss is {output_dict['sim_loss']}")


        if 'gram_loss' in output_dict:
            print(f"---------detection loss is {total_loss}")
            total_loss = total_loss + 2*output_dict['gram_loss']
            print(f"---------gram_loss is {output_dict['gram_loss']}")
