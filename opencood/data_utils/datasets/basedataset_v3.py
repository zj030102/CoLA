    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        if cur_ego_pose_flag:
            transformation_matrix = x1_to_x2(delay_cav_lidar_pose,
                                             cur_ego_lidar_pose)
            right_transformation_matrix = x1_to_x2(delay_params['lidar_pose'], cur_ego_lidar_pose)
            spatial_correction_matrix = np.eye(4)
        else:
            transformation_matrix = x1_to_x2(delay_cav_lidar_pose,
                                             delay_ego_lidar_pose)
            right_transformation_matrix = x1_to_x2(delay_params['lidar_pose'], delay_ego_lidar_pose)
        ...
        delay_params['right_transformation_matrix'] = right_transformation_matrix  # 未经噪声扰乱的正确他车-> ego的转换矩阵
