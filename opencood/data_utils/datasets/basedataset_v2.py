...
class BaseDatasetV2(Dataset):
    ...
    def retrieve_multi_data(self, idx, select_num, cur_ego_pose_flag=True):
        ...
        if self.async_mode == 'real':
            self.latency = [0] + [np.random.uniform(0, self.async_overhead) // 100 for _ in range(self.max_cav - 1)]
        else:
            self.latency = [0] + [self.async_overhead // 100 for _ in range(self.max_cav - 1)]
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():  
            ...
            # 此框架中retrieve_base_data仅用于提取当前帧数据用作非时序方案或作为时序方案的真值
            timestamp_delay = 0
            # print(f'base {timestamp_delay}')
    ...
    def retrieve_base_data_before(self, scenario_index, idx, cur_timestamp_key, cur_ego_pose_flag=True):
        ...
        i=0
        for cav_id, cav_content in scenario_database.items(): 
        # if cav_content['ego']:  #scope只需要ego车的历史帧
            data[cav_id] = OrderedDict()
            data[cav_id]['ego'] = cav_content['ego']

            # calculate delay for this vehicle
            timestamp_delay = int(self.latency[i]) 
            # print(f'before {timestamp_delay}')
