...
class PillarVFE(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        points_mean = \
            ...
            voxel_num_points.type_as(voxel_features).view(-1, 1, 1) # 17179, 1, 3
        f_cluster = voxel_features[:, :, :3] - points_mean  # 17179, 32, 3 距离几何中心的offset

        f_center = torch.zeros_like(voxel_features[:, :, :3])
        f_center[:, :, 0] = voxel_features[:, :, 0] - (
                coords[:, 3].to(voxel_features.dtype).unsqueeze(
                    1) * self.voxel_x + self.x_offset)  # 每个点的真实lidar坐标m
